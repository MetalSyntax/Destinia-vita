...
static char current_log_path[256] = "";

static void init_file_logger(void) {
    sceIoMkdir("ux0:data/destinia", 0777);
    sceIoMkdir("ux0:data/destinia/logs", 0777);

    for (int i = 1; i <= 999; i++) {
        char path[256];
        snprintf(path, sizeof(path), "ux0:data/destinia/logs/game_%03d.log", i);
        SceIoStat stat;
    ...
    if (current_log_path[0] == '\0') {
        snprintf(current_log_path, sizeof(current_log_path), "ux0:data/destinia/logs/game_001.log");
        sceIoRemove(current_log_path);
    }

    FILE *f = fopen(current_log_path, "w");
    if (f) {
        fprintf(f, "=== DESTINIA PS Vita Log: %s ===\n", current_log_path);
        fclose(f);
    }
}

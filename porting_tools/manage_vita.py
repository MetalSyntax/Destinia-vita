...
VITA_PORT = 1337
VITA_TITLE_ID = "DESTINIA1"
# Carpeta donde build.sh/build_and_install.sh dejan los .vpk generados.
BUILD_DIR = "build"
VITA_DOWNLOADS_DIR = "/ux0:/downloads"
VITA_DATA_DIR = "/ux0:/data"
VITA_GAME_DIR = "ux0:/data/destinia"
VITA_LOGS_DIR = "/ux0:/data/destinia/logs"
VITA_CG_DIR = "ux0:/data/destinia/cg"
VITA_GLSL_DIR = "ux0:/data/destinia/glsl"
BASE_DIR = os.path.dirname(os.path.abspath(__file__))

def print_banner():
    # El banner ahora se imprime directamente en main() con colores
    pass
...
def choose_vpk():
    ...
    if not choice:
        return vpks[0]
    if choice.lower() in ['0', 'q', 'n', 'no', 'c', 'cancel', 'r', 'volver']:
        print("[*] Operación cancelada. Regresando al menú principal...")
        return None
    try:
        idx = int(choice)
...
def upload_eboot():
    ...
    size_kb = os.path.getsize(local_eboot_path) / 1024
    mtime = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(os.path.getmtime(local_eboot_path)))
    print(f"[*] eboot.bin encontrado: {size_kb:.1f} KB ({mtime})")
    
    choice = input(f"¿Deseas subir SOLO el eboot.bin a ux0:/app/{VITA_TITLE_ID}/? [S/n] (0 / 'q' para volver al menú): ").strip().lower()
    if choice in ['0', 'q', 'n', 'no', 'c', 'cancel', 'r', 'volver']:
        print("[*] Operación cancelada. Regresando al menú principal...")
        return
    ...
    dest_dir = f"/ux0:/app/{VITA_TITLE_ID}"
...
def download_latest_debug_files():
    choice = input("¿Deseas conectar a la PS Vita y buscar crash dumps y logs? [S/n] (0 / 'q' para volver al menú): ").strip().lower()
    if choice in ['0', 'q', 'n', 'no', 'c', 'cancel', 'r', 'volver']:
        print("[*] Operación cancelada. Regresando al menú principal...")
        return
    ...
    if descargar_dmp in ['s', 'y', 'si', 'yes']:
        ...
        try:
            ...
            if latest_dmp:
                ...
                local_dmp_name = os.path.join(logs_folder, f"Destinia-{latest_dmp}")
    ...
    print(f"[*] Buscando el último log de Destinia en {VITA_LOGS_DIR}...")
    latest_log = None
    latest_log_time = 0
    try:
        ...
        if latest_log:
            ...
            print(f"[-] No se encontraron archivos de registro (.txt) en {VITA_LOGS_DIR}.")
    except all_errors as e:
...
VITA_GAME_DATA_DIR = f"{VITA_GAME_DIR}/data"
LOCAL_DATA_REFERENCE_DIR = "destinia_extract/assets"
...
def verify_data_assets():
    ...
    el volcado local de referencia (destinia_extract/assets)
    contra lo que realmente está subido en ux0:data/destinia/data/ en
    la Vita. Es un chequeo SUPERFICIAL (no recursivo) a propósito."""
    project_root = os.path.dirname(BASE_DIR)
...
def main():
    options = [
        ...
        (f"Subir SOLO el eboot.bin a la PS Vita (ux0:app/{VITA_TITLE_ID}/)", upload_eboot),
        ("Descargar el último dump (.dmp) y log (.txt) de Destinia", download_latest_debug_files),

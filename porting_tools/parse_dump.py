...
def auto_find_files(root_dir):
    ...
    # Search SO
    so_candidates = [
        s for s in glob.glob(os.path.join(root_dir, "**", "*.so"), recursive=True)
        if not os.path.basename(s).startswith("._")
    ]
    if so_candidates:
        # Prioritize libdestinia_jni.so or libgame.so
        so_candidates.sort(key=lambda x: 0 if "libdestinia_jni" in x or "libgame" in x or "libmain" in x else 1)
...
def analyze_dump(dump_path, elf_path=None, so_path=None, forced_so_base=None, stack_depth=36):
    ...
    def resolve_address(addr):
        ...
        if notation.is_located():
            ...
            if elf and (mod_name.endswith(".elf") or mod_name in ["destinia", "so_loader"]) and seg_num == 1:
    ...
    for thread in crashed_threads:
        ...
        for x in range(-4, stack_depth):
            ...
            if data:
                ...
                if ".so" in resolved or ".elf" in resolved or "destinia" in resolved or "so_loader" in resolved:
